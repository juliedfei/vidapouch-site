...
function sanitizePouchItems(
  ...
  for (
    ...
    /*
     * Keep structural validation for a reservation,
     * but do NOT verify a live merchant offer or
     * current bottle price here. Waitlist submission
     * is not checkout.
     */
    if (
      !isNonEmptyString(
...
function buildReservationPricingFallback({
  selectedPlan,
  pouchItems,
}: {
  selectedPlan:
  ...
  return {
    ...
    planOverageTooltip:
      "Final pricing will be confirmed before checkout.",
...
export async function POST(
  ...
  try {
     ...
     * IMPORTANT:
     *
     * A waitlist reservation is not checkout.
     *
     * Do not call revalidatePouchItemsForCheckout()
     * here. A live retailer-price lookup must never
     * prevent a customer from saving their VidaPouch
     * routine and joining the waitlist.
     *
     * We can calculate an estimated pricing snapshot
     * from the already-selected item data. If even
     * that calculation is unavailable, save the
     * reservation using the base plan price and a
     * zero overage. Final pricing can be verified
     * later when actual checkout opens.
     */
    let pooledPricing:
      SearchPouchPooledPricing;
    ...
    try {
      ...
      console.warn(
        "VidaPouch waitlist pricing snapshot unavailable; saving reservation with base-plan fallback:",
        pricingError
      );

      pooledPricing =
        buildReservationPricingFallback({
          selectedPlan,
