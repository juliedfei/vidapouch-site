...
export async function POST(
  ...
  try {
    /*
     * IMPORTANT:
     * The interactive sidebar preview no longer
     * performs live retailer revalidation.
     *
     * Search already captured the product-cost inputs.
     * Re-querying every retailer on every pouch change
     * made the customer funnel depend on a fragile
     * external lookup. Checkout still attempts a live
     * server-side verification separately.
     */
    const pricing =
    ...
    console.error(
      "Unable to calculate pooled VidaPouch pricing preview:",
      error
    );

    /*
     * Return a customer-safe base-plan price instead
     * of failing the pricing funnel.
     */
    return NextResponse.json(
      {
        status:
